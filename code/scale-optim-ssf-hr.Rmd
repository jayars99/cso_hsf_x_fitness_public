---
title: "multi-scale pseudo-optimization for hourly SSF"
...
outline:
- univariate scale pseudo-optimization
- random effects structure:
  - random intercepts by individual and year, stratum-specific intercepts for each step
  - skip random slopes for now 
- pick top model based on AIC

### data
```{r}
ssf.hr.scales <- read_csv(here("data-public", "gps_ssf_hr_scales.csv")) %>%
...
cc.0 = glmmTMB(case_ ~ cc.0 + (1|step.id.id) + (1|id), 
                       family=poisson, data= ssf.hr.scales, doFit=FALSE) 
...
anova(cc.0, cc.30, cc.60, cc.120, cc.300, cc.500) # 30 is best
```
...
anova(lfd.0, lfd.30, lfd.60, lfd.120, lfd.300, lfd.500) # 120 is best
```
...
anova(s10z.30, s10z.60, s10z.120, s10z.300, s10z.500) # 60 is best
```
...
anova(sci.30, sci.60, sci.120, sci.300, sci.500) # 60 is best
```
...
anova(sdq.30, sdq.60, sdq.120, sdq.300, sdq.500) # 30 is best
```
...
anova(sf.0, sf.30, sf.60, sf.120, sf.300, sf.500) # 30 is best
```
...
anova(sq.30, sq.60, sq.120, sq.300, sq.500) # 30 is best  
```


## data set for official hourly SSF modeling
```{r}
ssf.hr.final <- read_csv(here("data-public", "gps_ssf_hr_scales.csv")) %>%
  dplyr::select(Territory, usfws_id, Year, burst_, case_, sl_, ta_, t1_, t2_, dt_, step_id_, cc.30, ch.30, lfd.120, s10z.60, sci.60, sdq.30, sf.30, sq.30) %>%
  mutate(across(cc.30:sq.30, ~ as.numeric(scale(.x)), .names = "{.col}.scaled")) %>%
  mutate(id = str_c(Territory, ".", Year)) %>%
  mutate(step.id.id = str_c(id, ".", step_id_)) # success

write_csv(ssf.hr.final, here("data-public", "ssf_hr_final.csv"))
```
